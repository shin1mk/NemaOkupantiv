...
final class vehicles_fuel_tanks_Cell: UITableViewCell {
    private let customImageView: UIImageView = {
        let imageView = UIImageView()
        imageView.contentMode = .scaleAspectFit
        imageView.image = UIImage(named: "icon-auto.png")
        return imageView
    }()
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        setupConstraints()
        updateTitleLabelWithFuelTanksData()
    }
    ...
    func updateTitleLabelWithFuelTanksData() {
        WarshipAPIManager.shared.fetchWarfareData { result in
            switch result {
            case .success(let data):
                do {
                    let decodedData = try JSONDecoder().decode(WarfareData.self, from: data)
                    let Stats = decodedData.data.stats.vehicles_fuel_tanks
                    let Increase = decodedData.data.increase.vehicles_fuel_tanks

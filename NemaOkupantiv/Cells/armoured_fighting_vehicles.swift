//
//  armoured_fighting_vehicles.swift
//  NemaOkupantiv
//
...
final class armoured_fighting_vehicles_Cell: UITableViewCell {
    private let customImageView: UIImageView = {
        let imageView = UIImageView()
        imageView.contentMode = .scaleAspectFit
        imageView.image = UIImage(named: "icon-bbm.png")
        return imageView
    }()
    ...
    private let subtitleLabel: UILabel = {
        ...
        label.text = "Бойові броньовані машини"
        label.textAlignment = .left
        label.textColor = .black
        return label
    }()
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        setupConstraints()
        updateTitleLabelWithArmouredData()
    }
    ...
    func updateTitleLabelWithArmouredData() {
        WarshipAPIManager.shared.fetchWarfareData { result in
            switch result {
            case .success(let data):
                do {
                    let decodedData = try JSONDecoder().decode(WarfareData.self, from: data)
                    let Stats = decodedData.data.stats.armoured_fighting_vehicles
                    let Increase = decodedData.data.increase.armoured_fighting_vehicles

...
final class warships_cutters_Cell: UITableViewCell {
    private let customImageView: UIImageView = {
        let imageView = UIImageView()
        imageView.contentMode = .scaleAspectFit
        imageView.image = UIImage(named: "icon-ship.png")
        return imageView
    }()
    ...
    private let subtitleLabel: UILabel = {
        ...
        label.text = "Військові кораблі та катери"
        label.textAlignment = .left
        label.textColor = .black
        return label
    }()
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        setupConstraints()
        updateTitleLabelWithWarshipData()
    }
    ...
    func updateTitleLabelWithWarshipData() {
        WarshipAPIManager.shared.fetchWarfareData { result in
            switch result {
            case .success(let data):
                do {
                    let decodedData = try JSONDecoder().decode(WarfareData.self, from: data)
                    let Stats = decodedData.data.stats.warships_cutters
                    let Increase = decodedData.data.increase.warships_cutters
